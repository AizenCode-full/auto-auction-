...
export const RegisterModal: React.FC<RegisterModalProps> = ({ isOpen, onClose, onSwitchToAuth }) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const dataToSend = {
      ...
      inn: activeTab === 'fiz' ? formData.get('inn_fiz') : formData.get('inn_yur')
    };

    console.log('Данные для отправки регистрации:', dataToSend);
    login(); 
    onClose(); 
  };
  ...
  return (
    ...
    >
      ...
      >
        <button 
          type="button" 
          className="absolute top-6 right-6 bg-none border-none text-24px text-[#a0a0a0] cursor-pointer line-none leading-none hover:text-[#333] transition-colors" 
          onClick={onClose}
        >
          &times;
        </button>
        ...
        <form className="flex-grow flex flex-col justify-between" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-6">
            ...
            {activeTab === 'yur' && (
              <div className="w-full flex flex-col">
                ...
                <a 
                  href="#" 
                  className="inline-block text-xs text-[#163C66] underline mt-1 ml-1 hover:no-underline" 
          ...
          <button 
            type="submit" 
            className="border-none rounded-[10px] w-[404px] h-14 bg-[#163C66] text-white font-semibold text-base cursor-pointer mx-auto block hover:bg-[#0f2b4b] transition-colors"
          >
            Зарегистрироваться
          </button>
        </form>
