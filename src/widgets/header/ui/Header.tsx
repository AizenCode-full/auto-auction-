    ...
    <>
      ...
      <header className="w-full h-[108px] bg-white border-b border-[#1c426d]/10 flex justify-center items-center box-border relative z-40">
        <div className="w-[1440px] h-full flex items-center px-10 box-border justify-between">
          ...
          {/* Кнопка-гамбургер, Логотип, Счётчик торгов */}
          <div className="flex items-center flex-grow">
            ...
            <div className="text-sm text-[#a0a6b5] font-sans hidden sm:block">
          ...
          <div className="flex items-center pr-0 lg:pr-[130px] relative">
            ...
            {isAuth ? (
              <div className="flex items-center gap-4 relative">
                ...
                >
                  <img src="/icon-user.svg" alt="Профиль" className="w-13 h-13 object-contain" />
                </button>
              ...
              <button 
                type="button" 
                className="w-[180px] h-[47px] bg-[#163C66] text-white border-none rounded-lg text-base font-normal cursor-pointer transition-colors duration-200 flex justify-center items-center hover:bg-[#0f2b4c]" 
                onClick={onOpenAuth}
              >
                Личный кабинет
              </button>
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 flex items-center justify-center ${isMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        ...
        <div className={`relative w-full max-w-[800px] px-6 text-center transition-all duration-300 transform ${isMenuOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-5'}`}>
          ...
          <nav className="flex flex-col gap-6 items-center">
            ...
            >
              <svg className="w-8 h-8 text-current shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="mt-16 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-left">
              <span className="block text-xs uppercase tracking-widest text-gray-400 font-bold">Горячая линия бесплатно по КР</span>
              <a href="tel:312705829393" className="text-xl font-black text-white no-underline hover:text-[#90cdf4] transition-colors"> +312 (705) 82 93 93</a>
            </div>
            ...
            <div>
              {isAuth ? (
                <button 
                  onClick={logout}
                  className="h-11 px-6 bg-red-600/20 text-red-400 border border-red-500/30 rounded-lg text-sm font-bold cursor-pointer hover:bg-red-600 hover:text-white transition-all flex items-center gap-2"
                >
                  Выйти из системы
                </button>
              ) : (
                <button 
                  ...
                  className="h-11 px-8 bg-white text-[#163C66] border-none rounded-lg text-sm font-bold cursor-pointer hover:bg-[#90cdf4] hover:text-[#163C66] transition-all shadow-md"
                >
                  Авторизация ➔
                </button>
              )}
